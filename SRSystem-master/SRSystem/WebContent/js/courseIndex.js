...
$(document).ready(function(){
		$.getJSON("GetAllCourses",function(json){
			...
			for(var i=0;i<json.length;i++){
				...
				html+="<span class='glyphicon glyphicon-tasks'></span>Course shift</button></td></tr>";
	...
	function deleteCourses(){
		...
		if(count==0){
			...
			if(confirm("Are you sure you want to delete the selected course？")==true){
		...
		$.getJSON("GetAllSections",{courseNo:courseNo},function(json){
			...
			if(json.length==0){
				...
				for(var i=0;i<json.length;i++){
					...
					html+="<td><button class='btn btn-default' onclick=\"showStudents("+json[i].sectionNo+");\">View course selection</button></td></tr>";
				}
			}
